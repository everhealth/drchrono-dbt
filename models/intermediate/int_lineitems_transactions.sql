    ...
    , final AS (
        ...
        FROM line_item_transactions AS lit
            LEFT JOIN line_items AS li
                ON lit.lit_line_item_id IS NOT NULL AND lit.lit_line_item_id = li.line_item_id
        ...
        FROM line_item_transactions AS lit
            LEFT JOIN line_items AS li
                ON lit.lit_line_item_id IS NULL AND lit.lit_appointment_id = li.appointment_id
            LEFT JOIN era_objects AS era ON lit.lit_era_id = era.era_id
        WHERE lit.lit_line_item_id IS NULL OR li.line_item_id IS NOT NULL
    )
